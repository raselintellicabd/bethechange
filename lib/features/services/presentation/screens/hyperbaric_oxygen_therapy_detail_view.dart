...
class HyperbaricOxygenTherapyDetailView extends StatelessWidget {
  const HyperbaricOxygenTherapyDetailView({super.key, required this.service});
  ...
  Widget build(BuildContext context) {
    ...
    final conditions =
        _firstWhere(sections, _isConditions) ?? _fallbackConditions();
    final topics = sections.where(_isTopic).toList();
  ...
  ServiceSection? _fallbackConditions() {
    if (service.addressedConcerns.isEmpty) return null;
    return ServiceSection(
      type: 'cards',
      title: 'Which Conditions Can Hyperbaric Oxygen Be Beneficial For?',
  ...
  bool _isOverview(ServiceSection section) {
    ...
    if (type != 'image_text') return false;
    return section.title.trim().isNotEmpty ||
        section.content.trim().isNotEmpty ||
        (section.contentHtml?.trim().isNotEmpty ?? false);
  }

  bool _isConditions(ServiceSection section) {
    return section.title.toLowerCase().contains('which conditions');
  }

  bool _isTopic(ServiceSection section) {
    if (_isConditions(section) ||
        section.isFeaturedTherapies ||
        section.isGettingStarted) {
      return false;
    ...
    return title.contains('hyperbarics') || title.contains('& hyperbaric');
  }
  ...
  Widget build(BuildContext context) {
    ...
    final blocks = blogContentBlocks(
      contentHtml: (section.contentHtml == null ||
              section.contentHtml!.trim().isEmpty)
          ? null
          : _withoutAnchors(section.contentHtml!),
      body: section.content,
      subtitle: section.title,
    );
