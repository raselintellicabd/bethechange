...
/// Website-matched Diabetes page (`/diabetes/`). Not shared with other conditions.
class DiabetesDetailView extends StatelessWidget {
  const DiabetesDetailView({super.key, required this.condition});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          _HeroIntro(
            heroHeading: condition.heroHeading ?? 'diabetes',
          ...
          if (approach != null)
            _ApproachBenefitsSection(
              section: approach,
              benefitsTitle: condition.benefitsTitle,
...
class _HeroHeadingLine extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        const Expanded(child: Divider(color: AppColors.brandPrimary, thickness: 1)),
        Container(
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 14),
          _IconGrid(labels: labels, icon: Icons.auto_awesome),
        ],
      ),
    );
  }
...
class _GettingStartedSection extends StatelessWidget {
  const _GettingStartedSection({required this.section});
